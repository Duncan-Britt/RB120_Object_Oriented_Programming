...
class TTTGame
  ...
  attr_reader :board, :human, :computer, :minimax_current_player
  attr_accessor :tournament_won, :choice
  ...
  def initialize
    ...
    @choice = nil
    @minimax_current_player = [@human, @computer]
  end
  ...
  def computer_moves
    ...
    minimax(temp_board, temp_turn_idx)
    board[choice] = computer.marker
  end

  def minimax(brd, temp_turn_idx)
    if brd.someone_won? || brd.full?
      p g_points(brd, temp_turn_idx)
      return g_points(brd, temp_turn_idx)
    end
    points = []
    moves = []
    brd.unmarked_keys.each do |move|
      board_state = brd.dup
      board_state[move] = minimax_current_player[temp_turn_idx].marker
      points << minimax(board_state, ((temp_turn_idx + 1) % 2))
      moves << move
    end
    puts "moves: #{moves}, points: #{points}"
    if minimax_current_player[temp_turn_idx].marker == computer.marker
      max_idx = points.index(points.max)
      @choice = moves[max_idx]
      return points[max_idx]
    else
      min_idx = points.index(points.min)
      @choice = moves[min_idx]
      return points[min_idx]
    end
  end

  def g_points(brd, tidx)
    case brd.winning_marker
    when minimax_current_player[tidx].marker
      10
    when minimax_current_player[(tidx + 1) % 2].marker
      -10
    else
      0
    end
