...
class Computer < Player
  ...
  # end
  # MAX BY WHICH CHOICE WILL ELIMINATE THE MOST WINNING_LINES FOR ENEMY
  def moves
    available_spots = board.unmarked_keys # => [1, 2, 3, ...]
    # binding.pry
    # board[board.unmarked_keys.sample] = marker
    line = board.class::WINNING_LINES.max_by do |line|
      overlap(line, my_spots)
    end
    best_spots = line.select do |e|
      available_spots.include?(e)
    end
    board[best_spots.sample] = marker
    # board.class::WINNING_LINES
    # board[chosen_spot] = marker
  end

  private

  def overlap(line, arr)
    arr.select { |e| line.include?(e) }.count
  end

  def my_spots
    board.squares.keys.select do |key|
      board.squares[key].marker == marker
    end
  end

  def enemy_spots
    board.squares.keys.select do |key|
      !board.unmarked_keys.include?(key) && !my_spots.include?(key)
    end
  end
end
...
class TTTGame
  ...
  def initialize
    ...
    @move_methods = [Proc.new { human_moves }, Proc.new { computer.moves }]
